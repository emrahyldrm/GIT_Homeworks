...
int main(void)
{
	...
	/*END_OF_VARIABLES*/
								...
								/*if enter a wrong selection number(1 2..7)..*/
	selection = Starting(); 	/*.. user will return start*/
	
								
								/*when selection is different from 7..*/
	while (selection != 7)		/*..the program is not close itself*/
		{
		Calculator(selection);
								/*after the calculating*/
		selection = Starting();	/*program return main menu until enter "7"*/
		}
	...
	switch (selection)
	{
		case 1: /*Conjunction Mode*/
			{
				...
				printf("\" %d and %d \" = %d\n", x, y, Conj(x, y) );				
				CalcFinish();	/*writing finishing info*/
			}
				break;
		...
		case 6: /*Calculator Test Mode*/
			{				
				...
				printf("Calculated for each possibility\n");
